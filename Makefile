...
E2E_COVERAGE_IGNORE_REGEX ?= (/agent/|/preview/|/bin/agent_observability_harness\.rs$$|/(clipboard|content_safety|diff|git|preview|repo_graph|runtime|search|system_preview|text_layout|tree)\.rs$$)
AGENT_COVERAGE_IGNORE_REGEX ?= /src/(app|clipboard|content_safety|diff|folding|git|lsp|lsp_process|lsp_process_unix|lsp_process_windows|main|navigation|preview|repo_graph|runtime|search|system_preview|text_layout|tree|ui)\.rs$$|/src/(bin|preview)/
...
e2e-tab-shell: e2e-self-test ## Exercise tab open/switch/close and ⌘P palette
	$(CARGO) build --locked
	$(PYTHON) scripts/e2e_tui.py target/debug/$(BINARY) --scenario tab-shell --artifact-dir $(E2E_ARTIFACT_DIR)
