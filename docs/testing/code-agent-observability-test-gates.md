...
状态：Core 测试基线、Codex/Claude Code/OpenCode/TraeX adapter L0/L1 gate、用户级 Hook Setup 事务 E2E，以及 Codex/Claude/TraeX SessionStart 与 OpenCode session.created real-agent canary 已实现；完整 real-agent matrix 尚未启用。
...
10. Hook Setup 必须覆盖全量预检、语义合并、幂等升级、双层备份、提交中途失败回滚、恢复摘要冲突和 symlink/reparse point 拒绝；安装器 `-y` 调用由脚本 E2E 阻断，真实配置事务另由最终 `latte-lens` binary E2E 阻断。
...
|---|---|---:|---:|---:|---|
| G0 | format、all-features check、Clippy、MSRV、默认 feature/production guard | required | required | required | 已实现并接入 CI |
| G1 | bounded model、identity、envelope、reducer、metadata、runtime 的确定性 UT | required | required | required | 已实现 core suites |
| G2 | fake adapter/provider/store/publisher 的组合 contract suite | required | required | required | 已实现 synthetic suites |
| G3 | 无 PTY 的进程内或子进程 synthetic vertical slice | not required | metadata slice | required | 已实现 runtime/App baseline |
...
当前已具备 all-features `make test`、`make ci`、UT 93% / production PTY E2E 85% / Agent Core 80% 三个独立 line coverage 门禁、Agent contract/state/metadata/transport suites、六条 runtime/App headless vertical slices，以及 Linux/macOS 的仓库浏览、Code Navigation 和独立 Agent PTY journey。MSRV 和 Windows job 使用 all-features；默认构建另有 production negative gate，证明 synthetic harness 只能通过 required feature 构建。

新增 Core 回归覆盖包括：exact workspace offline/live 隔离、写入侧 global capacity、24h/7d production retention 调度、空 workspace 容量回收、live/fallback 独立 deadline、StateRootMismatch、contract downgrade source-aware 重仲裁、provider round-robin/deadline/draining、16 receiver cap 先过滤后截断，以及 Windows named-pipe loopback（由 Windows CI 执行）。

这些证据证明 core seam、synthetic vertical slice，以及 Codex/Claude/OpenCode/TraeX 官方 payload 到 production CLI 的离线/live 链路。它们不证明真实 Agent binary 的完整 compatibility。L0 emitter 的完整故障矩阵、五平台 16 receiver 同时 fan-out 的性能分布、更多 provider/PTY 组合场景和 real-agent G5 仍按后续独立 slice 增量补强。

## 3. 按改动类型选择阻断门禁

| 改动范围 | 必须通过 | 额外要求 |
|---|---|---|
| identity/model/envelope/contract | G0 + G1 + G2 | privacy canary；production registry 无默认/fake decoder |
...
| Hook Setup、安装器交互与恢复 | G0–G3 | 隔离 HOME；故障注入回滚；POSIX/Windows installer E2E；不得覆盖并发用户修改 |
| production adapter/provider | G0–G5 | 单独设计、单独权限确认，不复用 fake 结论 |
...
PTY E2E 使用专用 binary，但不污染 production registry。当前落地路径：
...
tests/
  ...
  agent_metadata_integration.rs
  agent_state_integration.rs
  agent_runtime_e2e.rs
  agent_transport_contract.rs
scripts/
  agent_e2e_tui.py
~~~
...
| UT-REG-001 | registry 无默认 decoder | duplicate、unknown、descriptor mismatch、approved-only production registry |
...
| 层级 | 入口与边界 | 证明内容 | 不能证明 |
|---|---|---|---|
| L0 Emitter process contract | 精确执行 `latte-lens hook`；真实 stdin/stdout/stderr/exit | payload decode、输出纪律、deadline、fail-open，且不启动 TUI | Lens 已接收或 UI 已更新 |
...
| E2E-H-015 | emitter stdio/exit contract | `latte-lens hook` 在正常、unknown adapter 和参数错误时都在预算内 exit 0，stdout/stderr 为空 | 启动 TUI、warning、阻塞或读取真实 Agent HOME |
...
| E2E-H-019 | two Lens fan-out | 同一 EventId 被两个匹配 receiver 独立 ACK 和归并 | 单 owner、负载均衡只送一个 |
| E2E-H-020 | partial fan-out | 一个 receiver Busy/crash 时健康 Lens 保持 live，Hook 写 metadata，失败 Lens refresh 后看到摘要 | 把部分成功当成完整 live |
| E2E-H-021 | exact workspace | Lens 与 Hook 同一 canonical 目录才匹配；父目录 Lens 不接收子目录 Hook | Git root/路径前缀/ancestor 猜测 |
| E2E-H-022 | multiple sessions/agents | 同 workspace 不同 SubjectNamespace/SessionKey/AgentKey 独立展示和计数 | 因 cwd 相同而错误合并 |
...
- receiver crash、restart、generation 改变、stale manifest、heartbeat TTL 和正常退出删除 lease；
- 同 workspace 的 1/2/16 个 receiver fan-out、部分 ACK，以及第 17 个 receiver 的显式有界行为；
...
- package smoke 使用默认 production features，测试 binary 使用 required test feature，二者不能复用同一产物。

该 gate 与 fake E2E 同等重要：fake vertical slice 通过不能以 production 包含 fake 为代价。

## 10. G5：真实集成兼容 E2E

G5 只在某个 production integration 独立立项后启用。它验证的是“声明支持的 Agent 版本确实安装、触发并到达 Lens”，不参与 C0–C2 core 完成判断。Codex 与 Claude Code adapter 当前均已通过官方文档形状、production CLI metadata fallback、production CLI → live receiver，以及隔离 HOME/config + loopback mock backend 的真实 SessionStart 手工 canary。OpenCode adapter 已通过官方插件/本地源码形状、真实插件 artifact、production CLI offline/live，以及隔离 HOME + loopback server + 空 session.create 的 session.created canary。TraeX adapter 已通过文档/本地源码形状、checked-in `hooks.json`、production CLI offline/live，以及隔离 HOME/config + loopback mock backend 的 SessionStart canary。完整 turn/tool/subagent、版本与跨平台触发矩阵尚未实现。
...
4. 运行 `make ci` 和 `make coverage`，不降低 UT 93%、production PTY E2E 85% 与 Agent Core 80% 三个 floor。
...
- coverage-e2e：在同一 profile 中执行 Files/Git/Search/Code Navigation production PTY、全部 production Hook CLI E2E 与 Agent PTY，保持 85% line floor；分母以 Makefile 的 `E2E_COVERAGE_IGNORE_REGEX` 为准，包含 `app.rs`、folding/LSP/navigation、`main.rs` 与 `ui.rs` 的适用路径。live CLI case 允许在插桩消耗固定 5 ms ACK budget 时同时产生幂等 metadata fallback，但仍必须证明 receiver 收到 frame。
- coverage-agent：统计 G1–G3 的全部 `src/agent/**` 模块，保持 all-features 80% line floor；不得通过排除新 Agent 模块维持数字。
- package：production negative gate。
- Integration job：只有达到 required 状态的 real-agent G5 canary 才阻断对应 integration，不替代主 agent-headless/PTY job；Codex/Claude/OpenCode/TraeX L0/L1 contract 先由常规 test job 阻断。
...
1. 扩展 L0/L1 emitter 子进程的 missing/malformed/delayed ACK 与 N-1/N/N+1 deadline 矩阵。
2. 为 Windows named-pipe peer/ACL 和 replace/lock 增加原生 runner 断言；POSIX PTY 不能替代它。
3. 增加 provider queue saturation、dynamic contract downgrade 和多 chunk snapshot 的 headless permutations。
4. 增加 Gap→snapshot、workspace switch、键鼠/resize 的独立 PTY current-screen journeys。
5. 后续真实 adapter/provider 仍须独立立项并进入 G5；Codex/Claude/OpenCode/TraeX 的 real-agent canary 也不得用 fixture/core 通过推断兼容。
