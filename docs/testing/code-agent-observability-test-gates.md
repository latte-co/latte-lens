...
当前已具备 all-features `make test`、`make ci`、UT 93% / production PTY E2E 85% / Agent Core 80% 三个独立 line coverage 门禁、Agent contract/state/metadata/transport suites、六条 runtime/App headless vertical slices，以及 Linux/macOS 的独立 Agent PTY journey。MSRV 和 Windows job 使用 all-features；默认构建另有 production negative gate，证明 synthetic harness 只能通过 required feature 构建。
...
- coverage-e2e：在同一 profile 中执行 Files/Git/Search production PTY、全部 production Hook CLI E2E 与 Agent PTY，保持 85% line floor；live CLI case 允许在插桩消耗固定 5 ms ACK budget 时同时产生幂等 metadata fallback，但仍必须证明 receiver 收到 frame。
