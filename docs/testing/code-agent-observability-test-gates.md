...
当前已经具备 all-features `make test`、`make ci`、80% line coverage、十个 C0 contract tests，以及 Linux/macOS 的仓库浏览 PTY E2E。它们是测试卡点的基础，但还不能称为 Agent observability E2E。
...
4. 运行 `make ci` 和 `make coverage`，不降低全局 80% floor。
...
- coverage：G1–G3，保持 all-features 80% line floor；不得通过排除新 agent 模块维持数字。
