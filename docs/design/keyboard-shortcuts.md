...
| `Ctrl+E` | 内容预览区存在选区且终端 workspace manager（如 Herdr）可用时，打开「发送选区到 agent」选择器；默认以 `path:行号` 锚点 + 围栏代码组装，可直接键入一行 `▎` 注释、`Tab` 切换纯文本，确认后暂存进目标 agent 输入框但不提交（另可在拖拽选区途中轻点 `Ctrl` 武装，松开鼠标即打开选择器）。选择器内：直接打字写注释、`↑/↓` 选 agent、`Tab` 切模板、`Enter` 暂存、`Esc` 取消 |
...
| 条件 | 展示策略 |
| --- | --- |
| 宽度 < 96 且 Preview | 精简：滚动、复制/退出、折叠、导航、符号、查找、`m` 渲染切换（仅 Markdown）、tab、复制路径、退出 |
| 宽度 < 96 且非 Preview | 精简：移动、焦点、拖拽复制、退出、tab、刷新、复制路径、退出 |
| 宽度 ≥ 96 且 Preview | 完整：滚动、复制/退出、折叠、回车切换、导航、符号、Alt 点击、历史、查找、`m` 渲染切换（仅 Markdown）、tab、复制路径、退出 |
