...
- 首期 local IPC 的 latency/ACK/backpressure；
- Codex exact file change；
- subagent live capture；
- 四个 vendor 的生产安装器；
- 任意 Hook 都必然带完整 SessionRef。

这些证据只说明未来 adapter 具有可行的采集入口，不表示当前 core 已实现 Codex 或任何其他 Code Agent 集成。
...
聚合型运行时还说明“相同 Hook 格式”不等于“同一个 Agent”：TraeCLI/Traex 虽兼容 Codex Hook 形状，仍需要独立 SubjectNamespace 和能力声明。Lens 的复用单位是 decoder/helper，不是把不同产品折叠为同一个 subject。
...
Future adapter/emitter 优先尝试 local IPC：

1. 通过 CodeAgentAdapter 构造 AgentObservation。
2. LiveObservationPublisher 在总 deadline 内将其编码为 ObservationFrame 并发送。
3. 收到 ACK_ACCEPTED 后不再由 Hook 写 metadata；Lens 负责合并、限频更新 metadata。
4. endpoint 不存在、NACK、timeout 或 version mismatch 时，降级为只更新 Session Metadata Beacon。
5. 所有路径 exit 0。
...
5. 取得 install 级 live receiver owner lease。
6. 绑定仅当前用户可访问的 IPC endpoint。
7. 在 background runtime 中发现 provider instances，协商 InstanceContract，并启动各自的 snapshot/subscription handshake。
8. 发布 live receiver ready generation；provider 则分别发布自己的 instance epoch/readiness。
...
6. 删除 ready marker/endpoint。
7. 释放 live receiver owner lease。

Lens crash 后 endpoint/OS lock 失效，后续 Hook 自动降级为更新 metadata。首期方案不恢复 crash 前尚未写入 metadata 的 live-only events。
...
SubjectNamespace 表示被观察产品的 native identity namespace，例如 `openai/codex`、`anthropic/claude` 或 `trae/traex`。ObserverId 表示提供证据的入口，例如 `openai/codex-hook`、`opencode/server` 或 `terminal-runtime/socket`。两者都是 adapter 声明的稳定、有界命名空间字符串，不是 core enum；最长 64 字节，只允许小写 ASCII 字母、数字、`.`、`_`、`-` 和 `/`，第三方使用 `organization/name` 命名空间。Core 不按名称做 source-specific 分支。
...
struct WorkspaceMetadata {
    ...
    canonical_fingerprint: StableDigest,
    ancestor_fingerprints: BoundedVec<StableDigest, 32>,
    repo_fingerprint: Option<StableDigest>,
    worktree_fingerprint: Option<StableDigest>,
    last_observed_at: Timestamp,
    checksum: Checksum,
}
~~~

不保存 raw cwd 或绝对路径。Ancestor fingerprints 让 Lens 打开 monorepo 父目录时发现子 workspace session。超过 32 层时 truncated 并标 Partial。
...
写入使用同目录、no-follow 的独占 temp 文件，经过 checksum 和重新解析后 atomic replace。Metadata 是提示性索引，首期方案默认不对每次更新执行 fsync；损坏记录会被隔离或忽略，并标记为 Partial，不能阻塞 Agent。
...
### 7.2 Endpoint 与 owner

首期方案为每个 install 设置一个 live receiver owner：
...
Endpoint/ready marker 包含 install ID、protocol version、receiver generation 和 selected-workspace membership digest，不包含 event payload 或 raw path。

首期方案中，同一 install 只有一个 live receiver owner。第二个 Lens 可以加载 MetadataOnly rows，但不能同时声明 live completeness。多 Lens 实时路由留到后续版本。
...
任一 deadline 超时都降级为更新 metadata，并 exit 0。不得等待 Lens 启动、重试连接、拉起进程或打印 warning。

### 7.4 Workspace membership

Live receiver 只 ACK 当前 selected workspace membership 中的 WorkspaceHint。

selected workspace 可以匹配多个 physical workspace keys：

- exact selected root；
- descendant cwd/repo/worktree；
- nested repo/submodule；
- canonical symlink alias 去重。

无法证明 membership 时返回 NotMember，Hook 降级为更新 metadata。不能仅因 cwd 字符串前缀相似就接收事件。
...
ObservationDispatcher 是 Hook emitter 路径的具体类型，不定义 trait。Provider envelope 由 AgentRuntime 直接走同一 registry/contract validation，不经过 metadata fallback dispatcher。dispatch(event, contract, deadline) 的固定语义是：
...
3. Accepted 时返回 LiveAccepted，不由 emitter 写 metadata。
4. 其他结果通过 project_metadata 从 ValidatedEnvelope 生成 delta；Hook event 的同-session facts 先折叠成一个 delta，再调用 SessionMetadataStore.merge。
5. 缺少 SessionRef 时不创建 metadata，只返回 IgnoredNoSession。
6. 任何 I/O 失败都是有界结果，不 panic、不重试、不改变 Code Agent 的退出状态。

Emitter 侧 validation 只是尽早拒绝错误，不能替代信任边界。LiveObservationReceiver 在 ACK_ACCEPTED 前必须按 current-user peer、ObserverId、ObserverInstanceId、epoch 和当前 InstanceContract 重新执行 envelope validation；验证失败返回 Invalid/Rejected，不能把发送方附带的 authority 当成已授权事实。
...
当前只实现并验证 core，不实现任何真实 Code Agent 集成：

- 不创建 Codex、Claude Code、OpenCode、Traex production adapter/provider；
- 不修改任何 Code Agent 的 Hook/plugin 配置；
- 不实现 vendor-specific install/remove/doctor/status；
- 不提供面向真实 vendor 的 hook emit CLI；
- 不使用真实 session 作为 core 测试的必要条件。

接口验证只使用 tests/support 中的 FakeAdapter、FakeProvider、InMemoryMetadataStore、FakePublisher，以及合成 AgentObservation/SnapshotEnvelope/EventEnvelope/InstanceContract fixtures。这些 fake 不进入生产 registry，不暴露给用户。

未来的单个 Code Agent 或聚合运行时集成必须作为独立工作项，只能通过 CodeAgentAdapter、可选 ObservationProvider、LiveObservationPublisher 和独立安装器边界接入，不得修改 AgentState 或 UI 的 observer/subject-specific 分支。
...
src/
  agent/
    ...
    view.rs                AgentViewState projection
    runtime.rs             bounded request/completion、provider、expiry 与 generation

    metadata/
      model.rs             WorkspaceMetadata/SessionMetadata
      paths.rs             LATTE_HOME state-root resolver
      store.rs             SessionMetadataStore filesystem implementation
      retention.rs         Lens-side bounded cleanup

    transport/
      protocol.rs          ObservationFrame/LiveAck
      unix.rs              current-user-only Unix endpoint
      windows.rs           current-user named pipe
      owner.rs             live receiver lease/generation
~~~
...
- Future emitter 与 Lens 对同一 install ID 必须解析到同一 root identity；不一致时拒绝合并并报 StateRootMismatch。
...
- Future emitter 日志和 stdout/stderr 严格为空。
...
- 生产 registry 中没有任何真实 Code Agent adapter。
...
C0–C2 完成后，core 仍不注册任何 production adapter/provider。真实集成需要单独的设计、权限确认和验证计划。建议验证顺序保持：OpenCode native snapshot/event vertical slice → Codex Hook/app-server partial → 聚合型终端 read-only bridge（验证 observer/subject 分离与动态 authority）→ Claude/Traex partial adapter。聚合型 bridge 不是 OpenCode/Codex adapter 的替代品。
...
- monorepo ancestor fingerprint、nested repo、worktree、symlink alias。
...
17. 首期方案中每个 install 只有一个 Hook live receiver owner；第二个 Lens 只读取 metadata。Provider 的多 Lens 连接能力按各 InstanceContract 声明。
...
22. 当前里程碑只实现 core 与 test fakes，不注册任何 production adapter/provider。
...
- 单一 live receiver owner 限制多个 Lens 同时实时观察。
...
- Workspace fingerprint 隐私与 ancestor mapping 实现错误会漏 session。
...
- Live owner 冲突时第二 Lens 的 UX 是什么？
...
- 任何真实 Code Agent 已通过 CodeAgentAdapter 接入。
...
- 生产 registry、CLI 和配置中没有任何真实 Code Agent adapter/provider。
...
- [Traex 本地事件协议](/Users/bytedance/projects/byted/trae-x/codex-rs/protocol/src/protocol.rs:1767)
- [Traex 本地 Hook runtime](/Users/bytedance/projects/byted/trae-x/codex-rs/core/src/hook_runtime.rs:498)
