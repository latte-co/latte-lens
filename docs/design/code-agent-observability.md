...
- `latte-lens hooks setup` 作为独立、显式、可逆的产品动作，语义合并用户级 `.codex/hooks.json`；它不修改 `config.toml`、项目配置或 Hook 信任状态。
...
Claude Code 的 command Hook 使用 `${CLAUDE_PROJECT_DIR}` 表示 session project root。配置必须通过 exec-form `args` 把它传给 `--workspace`，避免 session 中途 `cd` 后把同一 Agent 错误归入另一个 Lens 工作区。`latte-lens hooks setup` 语义合并用户级 `~/.claude/settings.json` 或 `CLAUDE_CONFIG_DIR/settings.json`，不修改项目 `.claude/settings.json` 或 `.claude/settings.local.json`。完整配置与恢复边界见 [Claude Code Hooks 集成](../integrations/claude-code-hooks.md) 和 [Code Agent Hooks 安装与恢复](../integrations/hook-setup.md)。
...
src/
  ...
  agent/
    ...
    hook_setup.rs          用户级 Hook 配置事务、双层备份、回滚与恢复
