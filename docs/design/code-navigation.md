...
状态：实现中。结构快照、跨平台 LSP 进程边界、协议 actor、App 原子跳转、结果选择器与
历史导航已落地；真实语言服务器、Windows process-tree 和 PTY 端到端验证仍由交付测试门禁
确认。
...
- Content focus 且无非空 selection 时，对 caret 所在 grapheme 加下划线；它不是文本编辑
  光标，不接受字符输入。semantic shortcut 只查询后台产出的 `RecognizableTokenIndex`；index 不完整、空白、
...
- 绝对 program 和 basename 解析结果使用同一 trust validation：逐级 `symlink_metadata`
  ...
  再次确认仍为同一普通文件且不位于 `App::root` 内。Unix 记录 canonical file 的
...
- `NavigationSettings` 只保存 canonical absolute program、受限 args 和初次 identity。**每一次**
...
| 按键 | 行为 |
| --- | --- |
| `Char('b')` + modifiers 恰为 `CONTROL` 或 `SUPER` | Definition；对齐 IntelliJ 的 `Ctrl/Cmd+B` 语义 |
| `Char('b')` + modifiers 恰为 `CONTROL+ALT` 或 `SUPER+ALT` | Implementations；对齐 IntelliJ 的 `Ctrl+Alt+B` / `Option+Cmd+B` 语义 |
| `F7` + modifiers 恰为 `ALT` | References；对齐 IntelliJ 的 `Alt/Option+F7` 兼容键 |
| `Char('k')` + modifiers 恰为 `CONTROL` 或 `SUPER`，再按 `d` / `r` / `i` | 进入 4 秒 terminal-safe chord，分别请求 Definition / References / Implementations；`Esc` 取消 |
| `F12` + modifiers 恰为 `NONE` | Definition；保留 VS Code 兼容键 |
| `F12` + modifiers 恰为 `SHIFT` | References；保留 VS Code 兼容键 |
| `F12` + modifiers 恰为 `CONTROL` | Implementations；保留 VS Code 兼容键 |
...
不引入 `g d` chord，因为 Content 中 `g` 当前是 Home，Tree 中 `g` 当前选择首行；不改变既有单键
语义。`Ctrl/Cmd+K` 采用 VS Code 风格的两段 chord，但第二段使用明确的 `d/r/i` 终端字符，避免 macOS
顶排媒体键，也避免要求终端必须区分 `Ctrl+Shift`。终端无法上报 Command/Super 时，`Ctrl` 形式始终
作为等价入口。README 的 controls 表与 footer 同步展示 terminal-safe 入口。
...
刷新输入；再次请求会取消前一个。chord 等待第二段时，未知按键取消 chord 后继续按原语义处理，避免
吞键；4 秒超时或 `Esc` 只退出 chord，不触发应用退出。
...
所有 semantic shortcut 与 Ctrl/Super click 都只能在已排序、去重、互不重叠的 ranges 上使用
...
- config/trust：64 KiB/64 KiB+1、strict UTF-8/BOM、所有层 duplicate/unknown field、entry/arg/string
  ...
  默认命令缺失只禁用单 family、绝对 program、自定义 basename PATH、相对 PATH、symlink/reparse、workspace executable、Unix
...
- IntelliJ `Ctrl/Super+B`、`Ctrl/Super+Alt+B` 与 `Alt+F7`、terminal-safe `Ctrl/Super+K` 后接 `d/r/i`、chord
  timeout/Esc/未知键不吞键、F12 的 NONE/SHIFT/CONTROL 兼容矩阵、Alt 左右、@ 的 NONE/SHIFT、额外
  modifier 忽略；
...
1. 打开 caller Preview，鼠标点击/按 terminal-safe `Ctrl+B`（F12 仍保留兼容覆盖）；
2. 等待 `Starting rust-analyzer…` 与 definition target；
3. `Ctrl+K` 后按 `R` 打开多结果 picker，选择另一个文件；
4. Alt+Left/Alt+Right 返回/前进；
5. 无可用 server 时明确显示
   `Code navigation is unavailable for Rust: no language server was found.` 且绝不 spawn；非法
   workspace 内 executable、symlink/reparse 或 Windows shell-backed command 被拒绝；TUI 仍可滚动和退出；
...
- Apple [Use keyboard function keys on Mac](https://support.apple.com/guide/mac-help/use-keyboard-function-keys-mchlp2596/mac)
  说明顶排默认执行系统功能、需配合 Fn/Globe 才发送标准功能键；VS Code
  [Default Keyboard Shortcuts](https://code.visualstudio.com/docs/reference/default-keybindings) 使用 F12 / Shift+F12
  和 Cmd/Ctrl-click；JetBrains
  [Navigating through the source code](https://www.jetbrains.com/help/idea/navigating-through-the-source-code.html?keymap=primary_default_for_macos)
  使用 Cmd+B、Option+Cmd+B 与 Option+F7。Latte Lens 因此保留两套 IDE 兼容入口，并把不依赖 F 键的
  Ctrl+K chord 作为终端兜底，而不假设所有终端都会上报 Command/Super。
