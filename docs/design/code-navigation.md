...
workspace-relative path 分组；文件 group row 显示文件名、相对目录和该文件的结果数，展开后的 location
row 显示 1-based line/column 与单行摘要。排序先按 path，再按协议 range，重复 target 去重后才计算总数：
...
退化为单列结果列表，不挤压出不可读的预览。`↑/↓/PageUp/PageDown` 或单击只更新 selection；宽屏下
以独立、可取消的 generation 异步更新左侧预览，不替换主 Content、不改变 tree、caret、viewport 或
history。`Enter` 或 400 ms 内双击才接受 location 并执行原子导航；`Esc` 关闭 popup 后完整保持查询前
位置。group row 只展开/收起，不是可接受的导航 target。
...
- Definition 的 one-direct/many-popup；References 与 Implementations 的 one-or-many-always-popup；
  ...
  键盘/鼠标/double-click/Enter/Esc restore focus。
