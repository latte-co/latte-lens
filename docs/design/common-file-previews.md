...
| 缓存陈旧或碰撞 | 缓存 key 包含进程随机 SipHash、输入长度、格式、路径和输出 limits；每次仍从安全句柄读取并哈希当前字节。 |
...
- PDF 每页单独调用有 decompression limit 的文本提取，页间检查 output/deadline；
- DOCX 只读取 `[Content_Types].xml`、`docProps/core.xml`、`word/document.xml`，不解压媒体；
- 每次从安全句柄读取当前字节并计算进程随机 SipHash；命中 8-entry/2-MiB LRU 后跳过
  parser/decode。缓存只保存最终 bounded `PreviewContent`，不保存原始图片/PDF/ZIP；
...
验证日期：2026-07-21；worktree：`latte-lens/feat-file-preview-providers`。
...
- 串行完整 Rust suites：324 lib tests passed / 1 ignored，96/96 App TUI integration、10/10
  CLI，以及 Git/tree/repository/LSP/Agent contract suites 均通过；
- `coverage-unit`：93.22% lines（门槛 93%）；`system_preview.rs` 的常见被动格式、主动后缀、
  executable signature、unknown confirmation 和 disabled adapter 已纳入 Q1 分母；
- `coverage-e2e`：85.48% lines（门槛 85%）；29/29 production PTY、10/10 CLI、1/1
  Agent TUI。新增 `unified-system-open` 为 17 条断言，另有 `image-preview-fallback` 11 条；
...
- `make install`：通过；`/Users/bytedance/.cargo/bin/latte-lens` 报告 `0.1.0`，与本 worktree
  ...
  `da182cbc25160a4710f81d5a871b047f683c577eae393e5bbb4bdb75fd4ccc70`；
...
- `make coverage` 首轮因新增分类器把 unit coverage 拉到 92.55% 而失败；补充真实分支测试后，
  `coverage-unit` / `coverage-e2e` / `coverage-agent` 三个原始门槛 target 分别通过。未把失败
  改写成首轮全绿。

因此本记录明确区分：本次统一交互、真实 PTY、格式/安全/性能边界、覆盖率和本机安装均已
通过；原样聚合 `make ci` 不声明全绿，剩余失败保留为上述既有短时限并发波动证据。
