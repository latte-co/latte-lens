...
路径组件，不会把符号链接、FIFO、socket、device、目录或 Windows reparse point
交给 Provider。
...
`open_regular()`，并把 `absolute_path` 交给会重新打开路径的库。静态链接和特殊文件
仍会在 dispatch 前被拒绝，但 Latte Lens 无法消除该 Provider 后续从 dispatch 到
open 之间的竞争，也无法取消任意阻塞代码。安全要求严格的 Provider 必须直接消费
